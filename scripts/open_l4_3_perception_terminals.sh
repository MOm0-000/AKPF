...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/open_l4_3_perception_terminals.sh [S1|S2|S3|S4|S5]
...
Options:
  ...
  --no-cleanup           Do not pre-clean old Gazebo/SITL/MAVROS/L4 processes.
  --cleanup-only         Only clean matching processes/windows, then exit.
  --no-auto-cleanup      Do not clean automatically after L4 AKPF node exits.
  --auto-cleanup-delay SEC
                         Delay after L4 AKPF node exits before cleanup. Default: 15
...
case "${SCENE}" in
  ...
  *)
    echo "Unknown L4.3 scenario: ${SCENE}" >&2
    exit 2
    ;;
esac

cleanup_known_processes() {
  tc_cleanup_common_round TERM "L4.3"
  ...
  sleep 2
  tc_cleanup_common_round 9 "L4.3"
  ...
  tc_cleanup_xterms_round TERM "L4.3"
  sleep 1
  tc_cleanup_xterms_round 9 "L4.3"
}

print_leftovers() {
  tc_print_common_leftovers "L4.3"
...
if [[ "${PRE_CLEANUP}" == "true" || "${CLEANUP_ONLY}" == "true" ]]; then
  echo "Cleaning old L4.3/Gazebo/SITL/MAVROS processes..."
  cleanup_known_processes
...
tc_select_backend
tc_prepare_run_dir "l4_3_terminals_${SCENARIO_NAME}"
...
if [[ ! -x "install/l3_akpf_navigation/lib/l3_akpf_navigation/l3_akpf_node" ]]; then
  echo "Missing L3/L4 build. Run: source /opt/ros/humble/setup.bash && colcon build --packages-select l1_velocity_control l3_akpf_navigation l4_perception_mapping" >&2
  exit 2
fi
source /opt/ros/humble/setup.bash
source install/setup.bash
...
if [[ "${AUTO_CLEANUP}" == "true" ]]; then
  echo "L4.3 AKPF node exited; cleanup in ${AUTO_CLEANUP_DELAY_S}s..."
...
tc_launch_terminal "L4.3 T1 Gazebo ${SCENARIO_NAME}" "${gazebo_cmd}"
tc_sleep_between "${GAZEBO_DELAY_S}"
tc_launch_terminal "L4.3 T2 SITL ArduCopter" "${sitl_cmd}"
tc_sleep_between "${SITL_DELAY_S}"
tc_launch_terminal "L4.3 T3 MAVROS" "${mavros_cmd}"
tc_sleep_between "${MAVROS_DELAY_S}"
tc_launch_terminal "L4.3 T4 pointcloud bridge" "${bridge_cmd}"
tc_sleep_between "${BRIDGE_DELAY_S}"
tc_launch_terminal "L4.3 T5 local mapper" "${mapper_cmd}"
tc_sleep_between "${MAPPER_DELAY_S}"
tc_launch_terminal "L4.3 T6 perception AKPF ${SCENARIO_NAME}" "${mission_cmd}"

echo "L4.3 terminals opened for ${SCENARIO_NAME}."
