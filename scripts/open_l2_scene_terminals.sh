...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/open_l2_scene_terminals.sh [S1|S2|S3|S4|S5]
...
Options:
  ...
  --no-cleanup           Do not pre-clean old Gazebo/SITL/MAVROS/L2 processes.
  --cleanup-only         Only clean matching processes/windows, then exit.
  --no-auto-cleanup      Do not clean automatically after L1 scene check exits.
  --auto-cleanup-delay SEC
                         Delay after L1 scene check exits before cleanup. Default: 15
...
case "${SCENE}" in
  S1|S1_single_front_obstacle) SCENARIO_NAME="S1_single_front_obstacle" ;;
  S2|S2_narrow_gate) SCENARIO_NAME="S2_narrow_gate" ;;
  S3|S3_corridor) SCENARIO_NAME="S3_corridor" ;;
  S4|S4_table_or_low_obstacle) SCENARIO_NAME="S4_table_or_low_obstacle" ;;
  S5|S5_corner) SCENARIO_NAME="S5_corner" ;;
  *)
    echo "Unknown L2 scenario: ${SCENE}" >&2
    exit 2
    ;;
esac

cleanup_known_processes() {
  tc_cleanup_common_round TERM "L2"
  tc_kill_processes_with_text 'l1_velocity_node'
  sleep 2
  tc_cleanup_common_round 9 "L2"
  tc_kill_processes_with_text 'l1_velocity_node' 9
  tc_cleanup_xterms_round TERM "L2"
  sleep 1
  tc_cleanup_xterms_round 9 "L2"
}

print_leftovers() {
  tc_print_common_leftovers "L2"
  tc_print_processes_with_text 'l1_velocity_node'
}

if [[ "${PRE_CLEANUP}" == "true" || "${CLEANUP_ONLY}" == "true" ]]; then
  echo "Cleaning old L2/Gazebo/SITL/MAVROS processes..."
  cleanup_known_processes
...
tc_select_backend
tc_prepare_run_dir "l2_terminals_${SCENARIO_NAME}"
...
if [[ "${AUTO_CLEANUP}" == "true" ]]; then
  echo "L2 scene check exited; cleanup in ${AUTO_CLEANUP_DELAY_S}s..."
...
tc_launch_terminal "L2 T1 Gazebo ${SCENARIO_NAME}" "${gazebo_cmd}"
tc_sleep_between "${GAZEBO_DELAY_S}"
tc_launch_terminal "L2 T2 SITL ArduCopter" "${sitl_cmd}"
tc_sleep_between "${SITL_DELAY_S}"
tc_launch_terminal "L2 T3 MAVROS" "${mavros_cmd}"
tc_sleep_between "${MAVROS_DELAY_S}"
tc_launch_terminal "L2 T4 L1 velocity scene check" "${mission_cmd}"

echo "L2 terminals opened for ${SCENARIO_NAME}."
